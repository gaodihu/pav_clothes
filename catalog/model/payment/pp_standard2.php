<?php
class ModelPaymentPPStandard2 extends Model {
	...
	public function call($data) {
		...
		if ($this->config->get('pp_standard2_test') == 1) {
			...
			$user = $this->config->get('pp_standard2_sandbox_username');
			$password = $this->config->get('pp_standard2_sandbox_password');
			$signature = $this->config->get('pp_standard2_sandbox_signature');
		} else {
			$api_endpoint = 'https://api-3t.paypal.com/nvp';
			$user = $this->config->get('pp_standard2_username');
			$password = $this->config->get('pp_standard2_password');
	...
	public function getMethod($address, $total) {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE `geo_zone_id` = '" . (int)$this->config->get('pp_standard2_geo_zone_id') . "' AND `country_id` = '" . (int)$address['country_id'] . "' AND (`zone_id` = '" . (int)$address['zone_id'] . "' OR `zone_id` = '0')");

		if ($this->config->get('pp_standard2_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('pp_standard2_geo_zone_id')) {
			$status = true;
		...
		if ($status) {
			$method_data = array(
				...
				'terms'      => '',
				'sort_order' => $this->config->get('pp_standard2_sort_order')
			);
		}
